...
export default function AdminUserDetailScreen() {
  ...
  const activeIsControlledByEndDate = !!activationEndDate;
  const effectiveIsActive = activeIsControlledByEndDate
    ? activationEndDate >= today
    : isActive;

  const validate = () => {
    const warn = (msg: string) =>
      Platform.OS === 'web' ? (window as any).alert(msg) : Alert.alert('Required', msg);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const resolvedActive = endDate ? endDate >= today : isActive;
      ...
      if (isNew) {
        ...
        await updateUserProfile(placeholderId, {
          phone, membershipType, isActive: resolvedActive,
    ...
    <ScrollView style={styles.root} contentContainerStyle={styles.container} keyboardShouldPersistTaps="handled">
      ...
      <View style={styles.avatarPreview}>
        <View style={[styles.avatar, { backgroundColor: `${MEMBERSHIP_COLORS[membershipType]}33` }]}>
          <Text style={[styles.avatarText, { color: MEMBERSHIP_COLORS[membershipType] }]}>{initials}</Text>
        </View>
      ...
      <View style={styles.section}>
        ...
        <Label>Email</Label>
        <Field icon="mail-outline" placeholder="member@email.com" value={email} onChangeText={setEmail} keyboardType="email-address" autoCapitalize="none" />
      ...
      {/* Activation dates */}
      <View style={styles.section}>
        <DateInput label="Activation Start Date" value={activationStartDate} onChange={setActivationStartDate} required />
        <DateInput label="Activation End Date (blank = open-ended)" value={activationEndDate} onChange={setActivationEndDate} />
      </View>
      ...
      {/* Payment — existing members only */}
      {!isNew && (
        <View style={styles.section}>
          <Label>Payment</Label>
          <PaymentSection uid={existing!.uid} membershipType={membershipType} />
        </View>
      )}
      ...
      <View style={styles.section}>
        <View style={styles.toggleRow}>
          ...
          <Switch
            ...
            onValueChange={activeIsControlledByEndDate ? undefined : setIsActive}
            disabled={activeIsControlledByEndDate}
...
const styles = StyleSheet.create({
  ...
  saveBtn: {
    ...
    height: 42, backgroundColor: colors.secondary, borderRadius: 10,
    marginTop: 8, paddingHorizontal: 20, alignSelf: 'flex-end',
  },
  saveBtnText: { color: '#000', fontSize: 13, fontWeight: '800' },
  disableBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6,
    height: 36, borderRadius: 10, marginTop: 8,
    borderWidth: 1, borderColor: `${colors.error}55`, backgroundColor: `${colors.error}11`,
  ...
  enableBtn: {
    flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 6,
    height: 36, borderRadius: 10, marginTop: 8,
    borderWidth: 1, borderColor: `${colors.success}55`, backgroundColor: `${colors.success}11`,
    alignSelf: 'flex-start', paddingHorizontal: 16,
  },
